...
class ClienteServiceImpl(
   ...
   private val cache: CacheCliente
    ...
    override fun findByName(name: String): List<Cliente> {
        ...
        val clientes = repository.getByName(name)
        if(clientes.isEmpty()){
            logger.error { "No se encontraron clientes con el nombre: $name" }
            throw ClienteServiceExceptions.ClienteNotFoundException("No se encontraron clientes con el nombre: $name")
        }
        return clientes
    }
    ...
    override fun getById(id: UUID): Cliente {
        ...
        val cliente = cache.get(id)?.let {
            ...
            val clienteRepo = repository.getById(id)
            if (clienteRepo != null) {
                cache.put(clienteRepo.id, clienteRepo)
            }
            logger.info { "Cliente encontrado en repositorio: $clienteRepo" }
            clienteRepo
        }
        if (cliente == null) {
